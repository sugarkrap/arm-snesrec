...
void decode_65C816(CodeAddr ca)
{
  ...
  if (op == 0x89) { // bit #imm
    ...
    if (ca.M) {
      uint8_t v = INS_GETA0(ca.ins);
      E->load_sym(VR_ARG0, "regA", EW8);
      E->mov_reg_immw(VR_ARG1, v, EW8);
    } else {
      uint16_t v = INS_GETA10(ca.ins);
      E->load_sym(VR_ARG0, "regA", EW16);
      E->mov_reg_immw(VR_ARG1, v, EW16);
      cycles++;
    }
    E->alu_reg(EA_AND, VR_ARG0, VR_ARG1);
    E->cmp_imm(VR_ARG0, 0);
    E->set_eq_sym("Z_Flag");
    ADD_CYCLES(cycles);
  } else
  ...
  if (op == 0x6D) { // adc addr
    ...
    if (ca.M) {
      CALL_FUNCTION_STK("__READ8");
      E->mov_reg_reg(VR_ARG0, VR_TMP);
      E->load_sym(VR_ARG1, "regA", EW8);
      CALL_FUNCTION_STK("__ADC8");
      E->store_sym("regA", VR_TMP, EW8);
    } else {
      CALL_FUNCTION_STK("__READ16");
      E->mov_reg_reg(VR_ARG0, VR_TMP);
      E->load_sym(VR_ARG1, "regA", EW16);
      CALL_FUNCTION_STK("__ADC16");
      E->store_sym("regA", VR_TMP, EW16);
      cycles++;
    }
  ...
  if (op == 0x61) { // adc (dp, x)
    ...
    uint8_t offset = INS_GETA0(ca.ins);
    E->load_sym(VR_ARG0, "regDP", EW16);
    E->alu_imm(EA_ADD, VR_ARG0, offset);
    APPLY_IDX_OFFSET(VR_ARG0, VR_TMP, "regX");
    E->alu_imm(EA_AND, VR_ARG0, 0xFFFF);
    CALL_FUNCTION_STK("__READ16");
    E->load_sym(VR_ARG0, "regDBR", EW8);
  ...
  if (op == 0x0D) { // ora addr
    ...
    if (ca.M) {
      ...
      E->load_sym(VR_ARG0, "regA", EW8);
      E->alu_reg(EA_OR, VR_ARG0, VR_TMP);
      E->store_sym("regA", VR_ARG0, EW8);
    } else {
      CALL_FUNCTION_STK("__READ16");
      E->load_sym(VR_ARG0, "regA", EW16);
      E->alu_reg(EA_OR, VR_ARG0, VR_TMP);
      E->store_sym("regA", VR_ARG0, EW16);
      cycles++;
    }
    UPDATE_NZ_A(ca.M);
    ADD_CYCLES(cycles);
  ...
  if (op == 0x12) { // ora (dp)
    ...
    LOAD_DP(VR_ARG0, offset);
    CALL_FUNCTION_STK("__READ16");
    E->load_sym(VR_ARG0, "regDBR", EW16);
    E->alu_imm(EA_SHL, VR_ARG0, 16);
    E->alu_reg(EA_ADD, VR_ARG0, VR_TMP);
    if (ca.M) {
      CALL_FUNCTION_STK("__READ8");
  ...
  if (op == 0xDD) { // cmp addr, x (unreachable dup of the 0xDD case above)
    int cycles = 4;
    uint16_t address = INS_GETA10(ca.ins);
    E->load_sym(VR_ARG0, "regDBR", EW8);
    E->alu_imm(EA_SHL, VR_ARG0, 16);
    E->alu_imm(EA_ADD, VR_ARG0, address);
    APPLY_IDX_OFFSET(VR_ARG0, VR_TMP, "regX");
    if (ca.M) {
      CALL_FUNCTION_STK("__READ8");
    } else {
  ...
  if (op == 0xB9) { // lda addr, y (unreachable dup of the 0xB9 case above)
    int cycles = 4;
    uint16_t address = INS_GETA10(ca.ins);
    LOAD_DBR_ADDR(VR_ARG0, address);
    APPLY_IDX_OFFSET(VR_ARG0, VR_TMP, "regY");
